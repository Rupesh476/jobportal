...
const navigate = useNavigate();
    ...
    const submitHandler = async (e) =>{
        ...
        try {
            setLoading(true);
            const res = await axios.post(`${import.meta.env.VITE_JOB_API_ENDPOINT}/post`, input,{
                headers:{
  ...
  return (
    ...
    </div>
    {
                ...
                : <Button type='submit' className='w-full cursor-pointer'>Post New Job</Button>
                }
    {
        companies.length === 0 && <p className='text-xs text-red-600 font-bold text-center my-3'>*Please register a company first to post job.</p>
    }
        </form>
    </div>
  </div>
